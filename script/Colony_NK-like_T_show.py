...
for s in image_dir:
    ...
    for i in range(len(cell_type)):
        ...
        for data_point in data_points[i]:
            cv.circle(img, tuple(data_point), int(r), (195, 192, 192), -1)

    # Overlay NK-like T cells
    if file_names_group_1:
        i = cell_type_new.index(cell_type1)
        r = round(np.sqrt(np.median(area[i]) / np.pi), 1)
        for data_point in data_points[i]:
            cv.circle(img, tuple(data_point), int(r), (87, 139, 46), -1)
