...
for s in image_dir:
    ...
    # Create white image
    img = np.ones((x_max, y_max), np.uint8) * 255

    f = f"../result/{s}/Process_text/{s}_SCLC_A_show.jpg"
    cv.imwrite(f, img) #第一级目录下生成
    img = cv.imread(f)

    print("file_names_group_1:",file_names_group_1)
    contours_1=contours_list[image_cell_list.index(file_names_group_1[0])]
 

    cv.drawContours(img, contours_1,-1, (255,225,0),10)

    cell_type_new=[]
    for i in cell_type:
        cell_type_new.append(i.strip('"'))

    for i in range(len(cell_type)):
        r = round(np.sqrt(np.median(area[i]) / np.pi), 1)
        print(cell_type[i], ":", r)
        for data_point in data_points[i]:
            cv.circle(img, tuple(data_point), int(r), (192, 192, 192), -1)

    # Overlay SCLC-A cells
    if file_names_group_1:
        i = cell_type_new.index(cell_type1)
        r = round(np.sqrt(np.median(area[i]) / np.pi), 1)
        for data_point in data_points[i]:
            cv.circle(img, tuple(data_point), int(r), (255, 225, 0), -1)
